...
public record HotelAvailabilitySearchDto(
    ...
    @Schema(example = "1234Abd")
    String hotelId,

    @NotNull
    @FutureOrPresent
    @JsonFormat(pattern = "dd/MM/yyyy")
    @Schema(type = "string", pattern = "dd/MM/yyyy", description = "date in format dd/MM/yyyy", example = "01/07/2023")
    LocalDate checkIn,

    @NotNull
    @FutureOrPresent
    @JsonFormat(pattern = "dd/MM/yyyy")
    @Schema(type = "string", pattern = "dd/MM/yyyy", description = "date in format dd/MM/yyyy", example = "30/07/2023")
    LocalDate checkOut,

    @NotEmpty
    @Size(max=10)
    @Schema(example = "[1,4,34,35]")
    List<@Range(min = 0, max = 120) Integer> ages) {
